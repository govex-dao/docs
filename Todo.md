...
- [ ] seperate execution from finalization, mybe give 30 minutes grace period to execute
- [ ] policy rule at coin / tpye level for all actions ( including proposal time needed and multisig of dao AND OR policy)
- [ ] early resolution / ending of proposals using flip rate quantum like thing uncertainty
- [ ] priority queue for proposals
- [ ] file registry system ( wlaurs or onchain strings) multiple chunks per file, per chunk rules e.g sunset etc
- [ ] new rnadom action that fails if random returns value < x. allows you to set probability proposal fails so all markets price correctly. At least reject and winning market priced correctly. should also have random consts to make proposal pass ( if price gap with reject < x%) so skew isnt one sided
- nav market making ? order book integrated into DAO??
- dividentd commit in monorepo: 08152066014d2ef8d80e9df21978b1666fc00765
- [ ] second order proposals commit monorepo 0afd6d71b5bb73f6d907de59333bf830f51e6466
